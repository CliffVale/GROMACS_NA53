...
| 2026-09-06 (15) | **100 ns production run completed + results documented** — `research/reports/2026-09-06-na53-100ns-results.md`: na53_prod 2036720 finished 2026-09-06 08:45:37 CST; **18.315 ns/day, 1.310 h/ns, 19 h 39 m production wall** (faster than the smoke-era ~15 ns/day estimate — 100 ns now fits a single 95 h slot, no RESTART needed for 100 ns); structural metrics over 1501 frames — RMSD 0.74 nm mean / **1.03 nm final (still drifting at 100 ns, not converged)**, Rg 2.94 nm (compact, range 2.37–3.36), SASA 148.5 nm^2 stable, ~71 intra-DNA H-bonds, T 310.14 K healthy, Pressure mean ~2 bar (instantaneous range ±150+ bar normal for Parrinello-Rahman); **density term looks wrong (~2.6 kg/m³ mean, range crossing zero) — same ID-mismatch class as the earlier pressure bug, needs gmx energy interactive ID check on T3 before trusting the density panel**; RMSF hotspots residues 73–75 (1.43 / 1.19 / 0.96 nm) — 3′ terminus still the flexible tail; PCA mode 1 = 35.1%, mode 2 = 15.7% (two modes ≈51% of variance → few collective motions dominate). Archive copied on T3 to `archive/2026-09-06_100ns_prod/` (prod.xtc 1.62 GB, prod.edr 415 KB, prod.cpt 7.0 MB, 22 analysis xvg, 8 figures, SLURM captures). **Archive downloaded to laptop with the tarball + rsync commands in §8 of the report.** Known stale items: this host's repo still only has the smoke-era `analysis/energy_Pressure.xvg`; the T3 archive also contains the smoke job's `na53_prod_2033888.out/.err` which should not be auto-selected as 'latest prod log'. | research/reports/2026-09-06-na53-100ns-results.md, archive/2026-09-06_100ns_prod/ (on T3) |
