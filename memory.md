...
- 🔵 **Phase 6 (T3 cluster run) — smoke + pilot COMPLETE, audit done, 100 ns still PENDING**: after the prep-cwd + DA5 fixes, a 1 ns smoke AND a 15 ns pilot prod (job 2036720, 18.315 ns/day) completed end-to-end with analysis (22 xvg + 8 figures archived). **Run audit (2026-09-06, entry 16): the '100 ns' report is actually a 15 ns run** — raw .tpr/log prove 7.5 M steps; density/pressure/NVT-em energy terms were re-extracted by name (bug class fixed for good via `gmx_energy_lib.sh`); true Rg = 3.548 nm; 347 clusters → no dominant basin; RMSD still rising at 15 ns → **a real 100 ns still needs to be run** (two 95-h RESTART segments, ~6 d wall, or GPU).
  - Resume point: decide length/route (CPU 2-segment 100 ns vs GPU request vs replicas), then `submit --ns 100` **and verify** `grep NS_LENGTH slurm/jobs/03_prod_*.sbatch` + the `… steps, … ps` line once mdrun starts.
...
5. Leave §1 TL;DR accurate.| 2026-09-05 (12) | **Real-T3 log format decoded + dashboard T/P fix** — ground truth from the LIVE prod 2033888 log (scripts/prod.log, 51.7 KB, 51 `Step` headers): gmx 2024.4 conda `.log` writes a 2-col `Step Time` pair followed by a **labeled energy block in scientific notation** (values like `-4.71145e+06`), where **Temperature/Pressure are labeled scalars, NOT table columns** — the old cols-4/5 parser could never show them. The stdout capture `logs/mdrun_prod.log` does NOT exist for this run (progress lives entirely in the sidecar). Live dashboard now: (a) STAGE bar/step/sim verified against the exact real block format (49%, 245000/500000, 490 ps); (b) **PHYSICS reads T/P order-anchored** — T = last value after a row ending `Temperature`; P = 2nd value after `Pres. DC (bar) Pressure (bar)` (cols-4/5 fallback kept for older gmx). IMPORTANT context: prod 2033888 is NOT dead (earlier "died at 180 ps" was a stale-paste artifact) — it was RUNNING on cpn3203 at ~16 ns/day (xtc 42 MB), and analysis 2033889 queued behind it. | scripts/live_dashboard.sh |

| 2026-09-05 (13) | **Smoke chain completed + download/analysis tooling** — 1 ns smoke finished end-to-end: prod 2033888 done (xtc 103.9 MB), analysis done with **8 figures** (energy_terms, hbonds, rmsd, sasa, gyrate, pca, rmsf, summary_dashboard). Mystery solved: `logs/mdrun_prod.log` never appears because 03_prod.sbatch runs in scripts/ with no internal redirect — mdrun stdout (incl. final `Performance:` line) lands in **logs/na53_prod_<jobid>.out** (SLURM capture). Monitor now merges that file (cf048a3: bar + ETA work for sbatch prod). Added `scripts/summarize_analysis.sh` — dependency-free summarizer of an analysis/ dir (ns/day from SLURM capture, xvg mean/min/max/stdev/final, RMSF top-3 residues, PCA variance, cluster occupancy) so results can be interpreted without pasting figures. Measured rates: NPT2 15.885 ns/day (28 cores), prod ~15 ns/day (56 cores) → **100 ns ≈ 6.7 d = 2 walltime segments** (59 + 41 ns via RESTART=1); 500/1000 ns = 9/17 segments (~35 d / ~67 d) — impractical on CPU ct56. 4 ns run submitted (--ns 4). | scripts/summarize_analysis.sh, scripts/live_dashboard.sh |

| 2026-09-05 (14) | **First results documented in repo** — `research/reports/2026-09-05-na53-t3-first-production-results.md` captures everything the study produced so far: smoke chain timeline (jobs 2033887-89 done, duplicate 2034005-08 cancelled, 4 ns never actually submitted), measured throughput (**NPT2 15.885 ns/day @28 cores; prod ~15 ns/day @56 cores — no 28→56 scaling gain**; 1 ns ≈ 1.6 h), full 1 ns structural table (RMSD 0.65 drifting/not converged, Rg 2.81 nm compact folded domain, ~71 intra-DNA H-bonds, stable SASA 149 nm^2, 3' tail floppy — res 73-75 up to 1.12 nm RMSF), the energy-ID bug (23/24/10/21 → virial + Coul. recip; fix f217451 to 17/36/11/14; smoke energy figures stale because prod.edr deleted), and the ETA comparison table 1/4/15/100/500/1000 ns (15 ns ≈ 25.5 h one job; 100 ns ≈ 6.7 d = 2 segments; 500/1000 ns impractical on CPU). 15 ns chain submitted. | research/reports/2026-09-05-na53-t3-first-production-results.md |
| 2026-09-06 (15) | **Run completed + results documented (LATER CORRECTED TO 15 ns — see entry 16)** — `research/reports/2026-09-06-na53-15ns-results-superseded.md`: na53_prod 2036720 finished 2026-09-06 08:45:37 CST; 18.315 ns/day, 19 h 39 m wall. **⚠️ This entry and its report mislabeled a 15 ns run as '100 ns' — entry (16) + `2026-09-06-na53-15ns-run-audit.md` are the corrections.** | research/reports/2026-09-06-na53-15ns-results-superseded.md (superseded) |
| 2026-09-06 (19) | **Permanent energy-sanity guard added (E-class closed)** — new `gmx_energy_check <xvg> <label> <lo> <hi> [unit]` in `scripts/gmx_energy_lib.sh` verifies the **steady-state (second-half) mean** of every extracted energy xvg sits in a physical window (T ∈ [300,320] K ref 310.15; ρ ∈ [950,1050] kg/m³; P ∈ [−100,100] bar ref 1.0; potential < −1000 kJ/mol). Wired into `02_equilibration.sh` (EM potential, NVT T, NPT2 ρ/T/P — 5 gates) and `04_analysis.sh` (prod T/ρ/P/Potential — 4 gates); under `set -euo pipefail` a violation **aborts the job** so a wrong term or unphysical run can never again flow silently into figures (E-class = 3 prior hits). Verified against BOTH corrected files (all ✓: T 310.1–310.3, ρ 988.6–988.7, P 2.2–2.5, Pot −4.09e6/−4.82e6) and pristine bug-era files (all ❌: Pres-XY 1.4 as density, Conserved-En −4.09e6 as T, pV 178.8 as density, density 988.7 as pressure, Coul.-recip +6.5e4 as EM potential). Regression guard C6 added to `scripts/check_repo_integrity.sh` (asserts lib function + ≥3 calls per stage — CI runs on every push); INCIDENT_ANALYSIS.md gains class E (taxonomy, register E1–E3, prevention, detection-gap #5). | scripts/gmx_energy_lib.sh, scripts/02_equilibration.sh, scripts/04_analysis.sh, scripts/check_repo_integrity.sh, docs/INCIDENT_ANALYSIS.md |
