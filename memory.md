...
| **Current phase** | Phase 6 (T3 cluster run) IN PROGRESS — env CREATED + doctor PASSED on T3; 1 ns smoke chain submitted 19:54 but **prep job 2031187 FAILED (~2 min, empty topol.top = pdb2gmx died)**. Root cause UNCONFIRMED — need `~/logs/na53_prep_2031187.out/.err`. Likely launcher-submits-from-repo-root vs templates-assume-slurm/ cwd mismatch (logs went to `~/logs/` not repo `logs/`). **Resume = diagnose prep log → fix → resubmit smoke** |
